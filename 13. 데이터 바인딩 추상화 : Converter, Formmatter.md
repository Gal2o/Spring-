...
- ## Spring Boot - WebConversionService
  - #### Spring Boot에서는 웹 애플리케이션인 경우, DefaultFormattingConversionService를 상속하여 만든 <br><br> `WebConversionService`를 `Bean을 등록해 준다.`
  ``` java
    @Component
  ...
  ```
  - #### DefaultFormattingConversionService이 아닌 WebConversionService를 사용하고 있다.
